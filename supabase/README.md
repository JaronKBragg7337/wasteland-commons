...
Provisioning is intentionally deferred until the owner confirms the organization and the creation cost. Once a dedicated project exists, apply both migrations in order, inspect the RLS/security advisors, exercise the lease/RPC and private Realtime room, and add the project URL/key only through deployment secrets.
